...
class Database:
    ...
    def close(self) -> None:
        ...
        if self.conn:
            try:
                ...
                # In some async/threaded contexts FastAPI may finalize dependencies
                # on a different worker thread than the one that created the
                # connection. With check_same_thread=False this should be allowed,
                # but certain environments can still raise ProgrammingError on
                # close(). Since the process is short-lived and connections are
                # per-request, swallow close-time thread errors to avoid breaking
    ...
    def cleanup_old_frames(self, retention_days: int) -> int:
        ...
        logger.info(
            "old_frames_cleaned", deleted=deleted, retention_days=retention_days
        )
        return deleted
